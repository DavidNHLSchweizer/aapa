...
class CRUD:
    """ Basic CRUD functions to connect a class to the database.
        ...
        Subclasses can handle more complicated cases. 
            see also: 
                ExtendedCRUD for tables with master/detail records tables attached

                DetailRecsCRUD for detail records
        ...
        ----------
            ...
            autoID: whether the associated class uses AAPA-generated keys for the table
                ...
                most classes do this. this guarantees that higher ID means later 
                creation, SQLite does not guarantee this. 
                
                Also it means that keys can be created programmatically at any point in the processing.
                
            mapper: the TableMapper mapping object attributes to the table
        ...
        -------
            basic CRUD functions (subclasses can overload to define more complicated behaviours ---
                ...
                read_many(list[Key]): read a list of objects from the database (utility when a list of IDs has already been generated by a query)
...
class CRUDQueries:        
    ...
    def find_values(self, attributes: str|list[str], values: Any|list[Any], map_values = True, read_many=False)->list[AAPAclass]:
        ...
        return []
    def find_ids_where(self, where_attributes: str|list[str] = None, 
                          where_values: Any|list[Any] = None, 
                          where_operators: list[Ops] = None)->list[int]:
        where_str = f'{where_operators=} ' if where_operators else 'equal'
        self.__db_log('FIND_IDS_WHERE', f'where_attributes: {where_attributes} {where_str}where_values: {where_values} ')
        qb = self.query_builder
        wanted_attributes, wanted_values = self.__get_wanted_values(where_attributes, where_values)
        log_debug(f'\tFVW: {wanted_attributes=} {wanted_values=}')        
        result = qb.find_ids_from_values(
                    # where=qb.build_where_from_values(
                        attributes=wanted_attributes, values=wanted_values,#operators=where_operators,
...
class CRUDColumnMapper(ColumnMapper):
    # class to map an attribute of an object to a table. 
    # example: mapping Aanvraag.student to the Studenten table
