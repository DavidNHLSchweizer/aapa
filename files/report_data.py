...
class AanvraagDataXLS:   
    ...
    def __to_sheet_row(self, aanvraag: AanvraagInfo):
        ...
        return [aanvraag.fileinfo.timestamp, aanvraag.student.student_name, aanvraag.student.studnr, aanvraag.student.first_name, aanvraag.student.telno, aanvraag.student.email, 
                get_datum_str(aanvraag.datum, aanvraag.versie), get_versie_str(aanvraag.datum, aanvraag.versie), aanvraag.bedrijf.bedrijfsnaam, aanvraag.titel, str(aanvraag.status), str(aanvraag.beoordeling)]
    def number_rows(self):
        return self.sheet.max_row
    def close(self):
        self.writer.close()
        print(f'Wrote report to {self.xls_filename}.')
