...
def import_aanvraag(filename: str, storage: AAPStorage)->bool:
    result = False
    try:
        if (a := AanvraagReaderFromPDF(filename)):            
            storage.create_aanvraag(a.aanvraag) #TODO: check whether previously imported
            result = True
    except Exception as E:
        logging.error(f'Error importing {filename}: {E}')        
    return result
