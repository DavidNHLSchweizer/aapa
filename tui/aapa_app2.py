...
from textual.widgets import Header, Footer, Static, Button, RadioSet, RadioButton, Switch, Label
...
class AapaSwitcher(Static):
    def compose(self)->ComposeResult:
        with Vertical():
            yield Horizontal(Static('scan directory'), Switch(value=False))
            yield Horizontal(Static('scan excel'), Switch(value=True))
            yield Horizontal(Static('blackboard'), Switch(value=True))
