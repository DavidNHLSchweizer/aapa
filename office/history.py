...
class BeoordelingOordeelModifier(WordProcessor):
    ...
    #write grade to the file
        ROW_GRADE   = 5
        COL_VALUES  = 2
        if (table := self.__find_table(1)):
            BeoordelingOordeelModifier.__write_cell_value(table, ROW_GRADE,COL_VALUES, grade)
    def __write_history(self):        
        ROW_GENERAL   = 1
        COL_GENERAL   = 1
        if (table := self.__find_table(2)):
            BeoordelingOordeelModifier.__write_cell_value(table, ROW_GENERAL, COL_GENERAL, 'Beoordeling ingelezen uit Excel-bestand')
    def __find_table(self, table_no):
        if self.document.Tables.Count > table_no:
            return self.document.Tables(table_no)
        else:
            return None
    def process(self, doc_path, grade, save_pdf_name, preview=False):
        try:
            self.open_document(doc_path)
            self.__write_grade(grade)
            self.__write_history()
            if not preview:
                self.save_as_pdf(save_pdf_name)
            aangemaakt = 'aan te maken' if preview else 'aangemaakt'
            logPrint(f'Feedback file {aangemaakt}: {save_pdf_name}.')
        finally:
            self.close()
